...
void Practical4::createScene(void)
{
    // Create the ground with grass on it
 	Ogre::Plane plane;
    plane.normal = Ogre::Vector3::UNIT_Y;	// horizontal plane with normal up in y-direction
   	plane.d = 0;	//Plane passes through the origin i.e. plane at y = 0
    Ogre::MeshManager::getSingleton().createPlane("floor",
    Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, plane, 4500,4500,10,10,true,1,10,10,Ogre::Vector3::UNIT_Z);
	...
	// Create a new sceneNode for the cube, attach it to the rootSceneNode as a child of the scene manager
	...
	m_cubeObject = new PhysicsObject();

	// Now initialise all of it’s properties to match the graphics model of the sphere as in ballNode
	m_cubeObject->initialise(cubeNode, cubeLength/2, cubeNode->getPosition(), Ogre::Vector3(0,44.2,0));
	...
	// Now initialise all of it’s properties to match the graphics model of the sphere as in ballNode
	m_cubeObject->initialise(ballNode, ballRadius, ballNode->getPosition(), Ogre::Vector3(0,44.2,0));
	*/
